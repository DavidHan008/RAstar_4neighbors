...
function [map_with_path,path,g_score,iter,cost,fail,t] = RAstar_manh_breakTies_4neighbors(isWithTieBreak,map,Nb_iter_max,i_start,j_start,i_goal,j_goal)
...
g_score(IndStart) = 0;
f_score(IndStart) = manh_dist(i_start,j_start,i_goal,j_goal);
...
for j=J'  
    ...
    if g_score(Indj)==Inf
       g_score(Indj)=g_score(current) + lateral_cost;
       f_score(Indj)=g_score(Indj)+tBreak*manh_dist(j(1),j(2),i_goal,j_goal);
...
if g_score(IndGoal)~=Inf
    ...
    while (i_current~=i_start | j_current~=j_start) 
        ...
        [m,index_successor]=min(neighbour_values');
